#!/usr/bin/env python3
"""Evaluate Challenge 1B retrieval outputs and scoring-weight ablations."""
...
def evaluate_existing(root: Path, k: int) -> list[dict]:
    ...
    for collection, relevant in GROUND_TRUTH.items():
        output_path = root / collection / "challenge1b_output.json"
    ...
    for variant, (semantic_weight, keyword_weight) in WEIGHTS.items():
        ...
        for collection, relevant in GROUND_TRUTH.items():
            result = process_1b_collection(
                root / collection / "challenge1b_input.json",
